...
io.use(async (socket, next) => {
  ...
  try {
    ...
    const [rows] = await pool.query(
      'SELECT id, name, role, avatar_url, department FROM users WHERE id = ? AND is_active = 1',
      [decoded.id]
    );
...
io.on('connection', (socket) => {
  ...
  socket.on('send_global_message', async (data) => {
    try {
      ...
      const id = uuidv4();
      await pool.query(
        'INSERT INTO messages (id, content, sender_id, is_global) VALUES (?, ?, ?, 1)',
        [id, content.trim(), user.id]
      );
      const message = {
        id,
  ...
  socket.on('send_group_message', async (data) => {
    try {
      ...
      const id = uuidv4();
      await pool.query(
        'INSERT INTO messages (id, content, sender_id, group_id, is_global) VALUES (?, ?, ?, ?, 0)',
        [id, content.trim(), user.id, group_id]
      );
      const message = {
        id,
        content: content.trim(),
  ...
  socket.on('typing_global', () => {
    socket.to('global').emit('user_typing', { name: user.name, room: 'global' });
  });

  socket.on('typing_group', ({ group_id }) => {
    socket.to(`group:${group_id}`).emit('user_typing', { name: user.name, room: group_id });
  });
